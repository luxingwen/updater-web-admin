...
export async function getProgramVersionDetails(
  body: API.Version,
  options?: { [key: string]: any },
) {
...
export async function uploadProgramPackageFile(programUuid, file) {
  const formData = new FormData();
  formData.append('file', file);
  return request<API.Package>(`/api/v1/program/package/file/upload/${programUuid}`, {
    method: 'POST',
    data: formData,
  });
}

export async function deleteProgramPackageFile(
  programUuid,
  fileName,
  options?: { [key: string]: any },
) {
  return request<API.Package>(`/api/v1/program/package/file/delete/${programUuid}`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
...
export async function getProgramActionInfo(
  body: API.ProgramAction,
  options?: { [key: string]: any },
) {
  return request<API.ProgramAction>(`/api/v1/program/action/detail`, {
    method: 'POST',
    data: {
      ...body,
    },
